$(document).ready(function(){
    ...
    function obterSession(callback) {
        $.ajax({
            url: API_SESSION,
            method: 'GET',
            dataType: 'json',
    ...
    function deletarSessao() {
        $.ajax({
            ...
            success: function (dado) {
                if (dado.length > 0) {
                    const sessionID = dado[0].id;
                    $.ajax({
                        url: `${API_SESSION}/${sessionID}`,
                        method: 'DELETE',
                        success: function () {
                            window.location.href = '../index.html';
                        },
                        error: function () {
                            mensagem('Erro ao excluir usuario', true);
                        }
                    })
                }
            }
                        ...
                        success: function(dado){
                            ...
                            dado.forEach(usuario => {
                                ...
                                <div class="card" id="${usuario.id}">
                                    ...
                                    <div class="card__title">${usuario.nome}</div>
                                    <div class="card__subtitle">${usuario.cargo}</div>
                                    <div class="card__wrapper">
                                    <button class="card__btn" id="editarBtn">Editar</button>
                                    </div>
                                    </div>`;
                                }
                            });
                    ...
                    $.ajax({
                        ...
                        success: function(dado){
                            ...
                            <div class="card" id="${usuario.id}">
                                ...
                                <div class="card__wrapper">
                                    <button class="card__btn" id="editarBtn">Editar</button>
                                    <button class="card__btn card__btn-solid" id="excluirBtn">Excluir</button>
                                </div>
                                </div>`;
                            });
                            $('.cards').append(tabela);
    ...
    $("#formUser").submit(function (e) { 
        ...
        if(!nome || !email){
            mensagem("Insira os dados corretamente!", true);
            return;
        }
        const dadoUsuario = {nome, email};
        ...
        if(id){
           $.ajax({
            ...
            error: function (){
                mensagem('Erro ao cadastrar usuário', true);
            }
           });
        }
    });
    ...
    $(document).on('click','#editarBtn', function () {
        ...
        obterSession(function(session){
            if(confirm("Deseja atualizar este usuario ?")){
                if(confirm("Deseja atualizar o nome ?")){
                ...
                if(session.cargo === "ADM"){
                    if(confirm("Deseja atualizar o cargo ?")){
                        let cargo = prompt("Digite 1 - ADM, 2 - SUPERVISOR, 3 - ESPECIALISTA");
                        if(cargo){
                            switch(cargo){
                                case "1":
                                    cargo = 'ADM';
    ...
    obterSession(function (session) {
        $(".session").append(`
            <li>Nome: <strong>${session.nome}</strong></li>
            <li>Cargo: <strong>${session.cargo}</strong></li>
